 ...
 * @LastEditTime: 2020-05-22 10:56:09
 * @Description:
 */
...
const COMIC_TITLE =
  "body > div.chapter-view > div.wrap_last_head.autoHeight > div > h1 > a";
const COMIC_CHAPTER =
  "body > div.chapter-view > div.wrap_last_head.autoHeight > div > h2";
...
function getComicChapterUrl(cheerio) {
  return (
    cheerio(COMIC_DETAIL_PAGE_URI_SELECTOR)
      .map((idx, ele) => {
...
let downloadImage = async (imageUrl) => {
  ...
  chapterUrls = [];
  let imageSrc = $(COMIC_IMAGE_DETAIL_IMAGE_URI_SELECTOR)
    .map((idx, ele) => {
      return cheerio(ele).attr("src");
    })
    .get();
  let ComicTitle = $(COMIC_TITLE)
    .map((idx, ele) => {
      return cheerio(ele).text();
    })
    .get();
  let ComicChapter = $(COMIC_CHAPTER)
    .map((idx, ele) => {
      return cheerio(ele).text();
    })
    .get();
  let currentPage = $(COMIC_IMAGE_DETAIL_PAGES)
    .map((idx, ele) => {
      return cheerio(ele).text();
    })
    .get();
  currentPage = _.toNumber(
    _.get(_.split(_.get(_.split(currentPage, "/"), 0), "("), 1)
  );
...
let getChapterImageUrl = async (chapterLink) => {
  // if (
  //   chapterLink != "https://www.manhuadui.com/manhua/haizeiwang/438940.html"
  // ) {
  //   return [];
  // }
  let instance = await phantom.create();
  ...
  $ = cheerio.load(content);
  chapterUrls = [];
  let totalPage = $(COMIC_IMAGE_DETAIL_PAGES)
    .map((idx, ele) => {
      return cheerio(ele).text();
    })
    .get();
  totalPage = _.toNumber(
    _.get(_.split(_.get(_.split(totalPage, "/"), 1), ")"), 0, 0)
  );
    ...
    for (let startUrl of startUrls) {
      ...
      for (let chapterLink of chapterList) {
        let {
          ...
          ComicTitle,
          ComicChapter,
        } = await getChapterImageUrl(chapterLink);
